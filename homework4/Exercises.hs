...
lastTwoPlus100 :: [Integer] -> [Integer] -> Integer
lastTwoPlus100 xs ys = addThree 0 100 $ applyOnLast (+) xs ys
...
-- start condition of recursion, as specified if n<=0 return x
applyManyTimes 0 f x = x
-- apply method by calling (f x) and then call itself with one less
-- invocation n - 1
...
-- Using a help function see if a frequency matches 'n'
freqBool :: Eq a =>  [a] -> Int -> a -> Bool
freqBool xs n x = n == freq x xs

-- invoke freqBool partially
freqFilter :: Eq a => Int -> [a] -> [a]
freqFilter n xs = filter (freqBool xs n) xs
...
-}
-- My solution here is most likely not the desired one
-- since the desired one probalby involves pattern matching, can't
-- think of solution like that at the moment
ex752 = sndColumn
sndColumn :: [[a]] -> [a]
sndColumn = map (\x -> last $ take 2 x)
-- sndColumn [[]] = []
-- sndColumn [] = []
-- sndColumn ((x:y:z):xs) = y : sndColumn xs
{-
  5.3.
...
canonicalizePairs :: Ord a => [(a, a)] -> [(a, a)]
-- first filter if they're the same, then apply map
canonicalizePairs xs = map (\(x,y) -> if y < x then (y, x) else (x, y) ) $ filter (\(x, y) -> x /= y) xs
